...
from modapi.rest.holds.biz_logic import hold_biz_logic, release_by_mod_biz_logic, _release_status_from_submit_time
...
def test_release_mod_hold_freeze(mocker):
    ...
    assert result
    assert isinstance(result, HoldReleaseLogicRes)
    assert result.release_to_status == 4 or not result.visible_comments
