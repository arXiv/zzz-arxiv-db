...
def with_queue_filters(user: User, stmt, include_mod_archives: bool = True, exclude_mod_categories: bool = False):
...
def with_mod_filters(user: User,
                     stmt,
    ...
    if exclude_mod_categories and include_mod_archives:
        pass
    else:
        category_ors.append(SubmissionCategory.category.in_(mods_categories))
        category_ors.append(and_(SubmissionCategoryProposal.category.in_(mods_categories),
    ...
    if include_mod_archives:
        for archive in user.moderated_archives:
            ...
            category_ors.append(and_(SubmissionCategoryProposal.category.startswith(archive),
                                     SubmissionCategoryProposal.proposal_status == 0))

        if exclude_mod_categories:
            stmt = stmt.filter(or_(
                SubmissionCategory.category.not_in(mods_categories),
                and_(SubmissionCategoryProposal.category.not_in(mods_categories),
                     SubmissionCategoryProposal.proposal_status == 0)
            ))
