...
from modapi.database import Base
...
t_arXiv_black_email = Table(
    "arXiv_black_email", metadata, Column("pattern", String(64))
)


t_arXiv_block_email = Table(
    "arXiv_block_email", metadata, Column("pattern", String(64))
)
...
class ArXivTracking(Base):
    ...
    sword_id = Column(
        INTEGER(8), nullable=False, unique=True, server_default=text("'00000000'")
    )
...
t_arXiv_white_email = Table(
    "arXiv_white_email", metadata, Column("pattern", String(64))
)
...
t_tapir_error_log = Table(
    ...
    metadata,
    Column(
        "error_date", INTEGER(4), nullable=False, index=True, server_default=text("'0'")
    ),
    Column("user_id", INTEGER(4), index=True),
    Column("session_id", INTEGER(4), index=True),
    Column(
        "ip_addr", String(16), nullable=False, index=True, server_default=text("''")
    ),
    ...
    Column(
        "message", String(32), nullable=False, index=True, server_default=text("''")
    ),
    Column("url", String(255), nullable=False, server_default=text("''")),
...
class TapirNicknamesAudit(Base):
    ...
    creation_ip_num = Column(
        String(16), nullable=False, index=True, server_default=text("''")
    )
    remote_host = Column(String(255), nullable=False, server_default=text("''"))
...
class TapirStrings(Base):
    ...
    name = Column(
        String(32), primary_key=True, nullable=False, server_default=text("''")
    )
    module = Column(
        String(32), primary_key=True, nullable=False, server_default=text("''")
    )
    language = Column(
        String(32), primary_key=True, nullable=False, server_default=text("'en'")
    )
...
class ArXivArchives(Base):
    ...
    arXiv_groups = relationship("ArXivGroups")
...
t_tapir_save_post_variables = Table(
    ...
    Column(
        "presession_id",
        ForeignKey("tapir_presessions.presession_id"),
        nullable=False,
        index=True,
        server_default=text("'0'"),
    ),
...
class TapirUsers(Base):
    ...
    flag_internal = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
    ...
    flag_approved = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'1'")
    )
    flag_deleted = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
    ...
    flag_allow_tex_produced = Column(
        INTEGER(1), nullable=False, server_default=text("'0'")
    )
    flag_can_lock = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
...
class ArXivAuthorIds(TapirUsers):
...
class ArXivOrcidIds(TapirUsers):
...
class ArXivQueueView(TapirUsers):
...
class ArXivSuspiciousNames(TapirUsers):
    __tablename__ = "arXiv_suspicious_names"

    user_id = Column(
        ForeignKey("tapir_users.user_id"), primary_key=True, server_default=text("'0'")
    )
    full_name = Column(String(255), nullable=False, server_default=text("''"))


class ArXivSwordLicenses(TapirUsers):
...
class TapirDemographics(TapirUsers):
    ...
    country = Column(
        ForeignKey("tapir_countries.digraph"),
        nullable=False,
        index=True,
        server_default=text("''"),
    )
...
class TapirUsersHot(TapirUsers):
    ...
    last_login = Column(
        INTEGER(4), nullable=False, index=True, server_default=text("'0'")
    )
    second_last_login = Column(
        INTEGER(4), nullable=False, index=True, server_default=text("'0'")
    )
    number_sessions = Column(
        INTEGER(4), nullable=False, index=True, server_default=text("'0'")
    )
...
class ArXivDemographics(TapirUsers):
    ...
    original_subject_classes = Column(
        String(255), nullable=False, server_default=text("''")
    )
    flag_group_physics = Column(INTEGER(1), index=True)
    flag_group_math = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
    flag_group_cs = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
    flag_group_nlin = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
    flag_proxy = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    ...
    flag_group_q_bio = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
    flag_group_q_fin = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
    flag_group_stat = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
    flag_group_eess = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
    flag_group_econ = Column(
        INTEGER(1), nullable=False, index=True, server_default=text("'0'")
    )
    veto_status = Column(
        Enum("ok", "no-endorse", "no-upload", "no-replace"),
        nullable=False,
        server_default=text("'ok'"),
    )

    arXiv_categories = relationship("ArXivCategories")
...
class ArXivCategories(Base):
    ...
    arXiv_archives = relationship("ArXivArchives")
    arXiv_endorsement_domains = relationship("ArXivEndorsementDomains")


class ArXivQuestionableCategories(ArXivCategories):
...
class ArXivControlHolds(Base):
    ...
    origin = Column(
        Enum("auto", "user", "admin", "moderator"),
        nullable=False,
        index=True,
    ...
    tapir_users = relationship(
        "TapirUsers",
        primaryjoin="ArXivControlHolds.last_changed_by == TapirUsers.user_id",
    )
    tapir_users1 = relationship(
        "TapirUsers", primaryjoin="ArXivControlHolds.placed_by == TapirUsers.user_id"
    )
...
class ArXivDblp(ArXivDocuments):
...
class ArXivPaperPw(ArXivDocuments):
...
class TapirEmailTemplates(Base):
    ...
    __table_args__ = (Index("short_name", "short_name", "lang", unique=True),)
    ...
    tapir_users = relationship(
        "TapirUsers", primaryjoin="TapirEmailTemplates.created_by == TapirUsers.user_id"
    )
    tapir_users1 = relationship(
        "TapirUsers", primaryjoin="TapirEmailTemplates.updated_by == TapirUsers.user_id"
    )
...
class TapirSessionsAudit(TapirSessions):
    ...
    session_id = Column(
        ForeignKey("tapir_sessions.session_id"),
        primary_key=True,
        server_default=text("'0'"),
    )
...
class ArXivAdminMetadata(Base):
    ...
    document_id = Column(
        ForeignKey("arXiv_documents.document_id", ondelete="CASCADE"), index=True
    )
    ...
    document = relationship("ArXivDocuments")
...
class ArXivCrossControl(Base):
    ...
    status = Column(
        Enum("new", "frozen", "published", "rejected"),
        nullable=False,
        index=True,
...
class ArXivDblpDocumentAuthors(Base):
    ...
    author = relationship("ArXivDblpAuthors")
    document = relationship("ArXivDocuments")
...
class ArXivDocumentCategory(Base):
    ...
    arXiv_category_def = relationship("ArXivCategoryDef")
    document = relationship("ArXivDocuments")
...
class ArXivEndorsementRequestsAudit(ArXivEndorsementRequests):
    __tablename__ = "arXiv_endorsement_requests_audit"

    request_id = Column(
        ForeignKey("arXiv_endorsement_requests.request_id"),
        primary_key=True,
        server_default=text("'0'"),
    )
...
class ArXivJrefControl(Base):
    ...
    __table_args__ = (Index("document_id", "document_id", "version", unique=True),)
...
class ArXivMetadata(Base):
    ...
    document = relationship("ArXivDocuments")
    arXiv_licenses = relationship("ArXivLicenses")
    submitter = relationship("TapirUsers")
...
class ArXivShowEmailRequests(Base):
    ...
    __table_args__ = (Index("user_id", "user_id", "dated"),)
...
class ArXivSubmissions(Base):
    ...
    submitter_id = Column(
        ForeignKey("tapir_users.user_id", ondelete="CASCADE", onupdate="CASCADE"),
        index=True,
    )
    submitter_name = Column(String(64))
    submitter_email = Column(String(64))
    ...
    arXiv_licenses = relationship("ArXivLicenses")
    submitter = relationship("TapirUsers")
    sword = relationship("ArXivTracking")


class ArXivPilotDatasets(ArXivSubmissions):
    __tablename__ = "arXiv_pilot_datasets"

    submission_id = Column(
        ForeignKey("arXiv_submissions.submission_id"), primary_key=True
    )
...
class ArXivSubmissionAbsClassifierData(ArXivSubmissions):
    __tablename__ = "arXiv_submission_abs_classifier_data"

    submission_id = Column(
        ForeignKey("arXiv_submissions.submission_id", ondelete="CASCADE"),
        primary_key=True,
        server_default=text("'0'"),
    )
    json = Column(Text)
...
class ArXivSubmissionClassifierData(ArXivSubmissions):
...
class ArXivTopPapers(Base):
    ...
    from_week = Column(
        Date, primary_key=True, nullable=False, server_default=text("'0000-00-00'")
    )
    _class = Column(
        "class", CHAR(1), primary_key=True, nullable=False, server_default=text("''")
    )
    rank = Column(
        SMALLINT(5), primary_key=True, nullable=False, server_default=text("'0'")
    )
    document_id = Column(
        ForeignKey("arXiv_documents.document_id"),
        nullable=False,
...
class ArXivVersions(Base):
    ...
    version = Column(
        TINYINT(3), primary_key=True, nullable=False, server_default=text("'0'")
    )
    request_date = Column(
        INTEGER(10), nullable=False, index=True, server_default=text("'0'")
    )
...
class ArXivVersionsChecksum(ArXivVersions):
    __tablename__ = "arXiv_versions_checksum"
    __table_args__ = (
        ForeignKeyConstraint(
            ["document_id", "version"],
            ["arXiv_versions.document_id", "arXiv_versions.version"],
        ),
    )

    document_id = Column(
        MEDIUMINT(8), primary_key=True, nullable=False, server_default=text("'0'")
    )
    version = Column(
        TINYINT(3), primary_key=True, nullable=False, server_default=text("'0'")
    )
...
class TapirAdminAudit(Base):
    ...
    tapir_users = relationship(
        "TapirUsers", primaryjoin="TapirAdminAudit.admin_user == TapirUsers.user_id"
    )
    tapir_users1 = relationship(
        "TapirUsers", primaryjoin="TapirAdminAudit.affected_user == TapirUsers.user_id"
    )
...
class TapirEmailHeaders(Base):
    ...
    template_id = Column(
        ForeignKey("tapir_email_templates.template_id"),
        primary_key=True,
        nullable=False,
        server_default=text("'0'"),
    )
...
class TapirEmailMailings(Base):
    ...
    tapir_users = relationship(
        "TapirUsers", primaryjoin="TapirEmailMailings.created_by == TapirUsers.user_id"
    )
    tapir_users1 = relationship(
        "TapirUsers", primaryjoin="TapirEmailMailings.sent_by == TapirUsers.user_id"
    )
    template = relationship("TapirEmailTemplates")
...
class ArXivEndorsements(Base):
    ...
    endorsee = relationship(
        "TapirUsers", primaryjoin="ArXivEndorsements.endorsee_id == TapirUsers.user_id"
    )
    endorser = relationship(
        "TapirUsers", primaryjoin="ArXivEndorsements.endorser_id == TapirUsers.user_id"
    )
    request = relationship("ArXivEndorsementRequests")


class ArXivEndorsementsAudit(ArXivEndorsements):
    __tablename__ = "arXiv_endorsements_audit"

    endorsement_id = Column(
        ForeignKey("arXiv_endorsements.endorsement_id"),
        primary_key=True,
        server_default=text("'0'"),
    )
    session_id = Column(INTEGER(10), nullable=False, server_default=text("'0'"))
...
class ArXivOwnershipRequests(Base):
    ...
    endorsement_request_id = Column(
        ForeignKey("arXiv_endorsement_requests.request_id"), index=True
    )
    ...
    endorsement_request = relationship("ArXivEndorsementRequests")
    user = relationship("TapirUsers")


class ArXivOwnershipRequestsAudit(ArXivOwnershipRequests):
    __tablename__ = "arXiv_ownership_requests_audit"

    request_id = Column(
        ForeignKey("arXiv_ownership_requests.request_id"),
        primary_key=True,
        server_default=text("'0'"),
    )
    session_id = Column(INTEGER(10), nullable=False, server_default=text("'0'"))
...
class ArXivPilotFiles(Base):
    ...
    submission = relationship("ArXivSubmissions")
...
class ArXivSubmissionCategory(Base):
    ...
    submission_id = Column(
        ForeignKey(
            "arXiv_submissions.submission_id", ondelete="CASCADE", onupdate="CASCADE"
        ),
        primary_key=True,
        nullable=False,
        index=True,
...
class ArXivSubmissionCategoryProposal(Base):
    ...
    arXiv_category_def = relationship("ArXivCategoryDef")
    proposal_comment = relationship(
        "ArXivAdminLog",
        primaryjoin="ArXivSubmissionCategoryProposal.proposal_comment_id == ArXivAdminLog.id",
    )
    response_comment = relationship(
        "ArXivAdminLog",
        primaryjoin="ArXivSubmissionCategoryProposal.response_comment_id == ArXivAdminLog.id",
    )
    submission = relationship("ArXivSubmissions")
    user = relationship("TapirUsers")
...
class ArXivSubmissionNearDuplicates(Base):
    ...
    __table_args__ = (Index("match", "submission_id", "matching_id", unique=True),)
    ...
    matching_id = Column(
        INTEGER(11), primary_key=True, nullable=False, server_default=text("'0'")
    )
