...
gcloud compute firewall-rules create allow-$PREFIX-health-check \
        --allow tcp:$PORT \
...
gcloud compute instance-templates create-with-container $TEMPLATE \
       ...
       --tags=allow-$PREFIX-health-check \
...
gcloud compute health-checks create http $TYPE-$PREFIX-health-check \
       --check-interval=10s \
       --host=$SERVER_NAME \
       --port=$PORT \
       --request-path=/status
...
gcloud compute instance-groups managed create $MODAPI_MIG \
       --base-instance-name $TYPE-$PREFIX \
       --initial-delay=120s \
       --size 1\
       --template $TEMPLATE \
